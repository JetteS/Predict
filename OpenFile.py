...
def fitVariationalBayes(X,Y,sigma_g,sigma_e,f2,p,maxIters=250):
	...
	for k in range(maxIters):
		...
		for m in range(M):
			...
			s = sp.array([z+sigma_e/sp.sum(x**2) for z in sigma_beta])
			beta_bar = sp.array([be*beta_hat/(be+(sigma_e/sp.sum(x**2))) for be in sigma_beta])
			tau = sp.array([be*sigma_e/sp.sum(x**2)/(be+(sigma_e/sp.sum(x**2))) for be in sigma_beta])
