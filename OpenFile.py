...
def fitVariationalBayes(X,Y,sigma_g,sigma_e,f2,p,maxIters=250):
	...
	for k in range(maxIters):
		...
		for m in range(M):
			...
			beta_bar = sp.array([s*beta_hat/(s+(sigma_e/sp.sum(x**2))) for s in sigma_beta])
			tau = sp.array([s*sigma_e/sp.sum(x**2)/(s+(sigma_e/sp.sum(x**2))) for s in sigma_beta])
...
## Obtaining the index of the minimum
