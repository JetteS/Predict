...
def FASTconjugateGradientSolve( X, x0, b, float c1=1, float c2=1):
	cdef int M, N
	cdef float rsold, rsnew, norm, alpha, 
	cdef vector <float> x(N) 
	cdef vector <float> r(N)
	cdef vector <float> p(N)
	cdef vector <float> Ap(N)
...
def fitVariationalBayes(X,Y,sigma_g,sigma_e,f2,p,maxIters=250):
	...
	sigma_beta = sp.array([sigma_g/float(M) *(1-f2)/p, sigma_g/float(M)*f2/(1-p)])
	...
	for k in range(maxIters):
		...
		for m in range(M):
			...
			s = sigma_beta+sigma_e/sp.sum(x**2)
			beta_bar = sigma_beta*beta_hat/(sigma_beta+(sigma_e/sp.sum(x**2)))
			tau = sigma_beta*sigma_e/sp.sum(x**2)/(sigma_beta+(sigma_e/sp.sum(x**2))) 
...
V_chr_inv_Y = sp.empty((N,25), dtype = "single")

for chrom in range(1,26):
	X_chr = X[:,sp.array(Chromosomes != chrom)]
...
prospectiveStat = sp.zeros(30, dtype = "single")
uncalibratedRetrospectiveStat = sp.zeros(30, dtype = "single")
...
for t in range(0,30):
	...
	prospectiveStat[t] = (sp.dot(x,V_chr_inv_Y[:,int(chrom)-1])**2)/sp.dot(x,V_chr_inv_x)
	uncalibratedRetrospectiveStat[t] = float(N)*(sp.dot(x,V_chr_inv_Y[:,int(chrom)-1])**2)/(sp.sum(x**2)*sp.sum(V_chr_inv_Y[:,int(chrom)-1]**2))
...
for chrom in range(1,26):
	...
	boltLMMinf[sp.array(Chromosomes == chrom)] = float(N)*sp.dot(X_chr.T,V_chr_inv_Y[:,chrom-1])**2/(sp.sum(X_chr**2, axis=0)*sp.sum(V_chr_inv_Y[:,chrom-1]**2))/infStatCalibration
...
for f2 in f2_vec:
	for p in p_vec:
		...
		for CVfold in range(0,5):
			...
			beta_fit = sp.array(ans[0], dtype = "single")
			#y_resid = ans[1]
...
uncalibratedBoltLMM = sp.zeros(M, dtype = "single")
## Leave-one-chromosome-out (LOCO) to avoid proximal contamination
for chrom in range(26):
	...
	#beta_fit = ans[0]
	y_resid = sp.array(ans[1], dtype="single")

	for snp in sp.where(Chromosomes == chrom):
		uncalibratedBoltLMM[snp] = N*sp.dot(X[:,snp],y_resid)**2/(sp.sum(X[:,snp]**2)*sp.sum(y_resid**2))
