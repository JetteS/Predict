...
def conjugateGradientSolveChunks(hf5, x0, b, c1=1, c2=1, chunk_size=3000):
	...
	for chunk in range(0, N, chunk_size):
		X_chunk = h5f['X'][chunk:chunk + chunk_size]
	...
	while norm > 0.0005:
		...
		XXTx = sp.empty(N, dtype="single")
		for chunk in range(0, N, chunk_size):
			X_chunk = hf5['X'][chunk:chunk + chunk_size]
...
def evalfREML(logDelta, MCtrials, hf, Y, beta_rand, e_rand_unscaled, chunk_size=3000):
	...
	for t in range(0, MCtrials):
		...
		H_inv_y_rand[:, t] = conjugateGradientSolveChunks(h5f=h5f, x0=x0, b=y_rand[:, t], c2=delta, chunk_size=chunk_size)
	...
	e_hat_data = conjugateGradientSolveChunks(h5f=h5f, x0=x0, b=Y, c2=delta, chunk_size=chunk_size)
